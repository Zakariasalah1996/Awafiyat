...
recipes = json.loads((root / 'data/recipe-batch-7.json').read_text(encoding='utf-8'))['recipes']
images = json.loads((root / 'data/recipe-batch-7-images.json').read_text(encoding='utf-8'))
if len(recipes) != 20 or len(images) != 20:
    raise SystemExit(f'Expected 20 recipes and images, got {len(recipes)} and {len(images)}')
entries = []
for index, recipe in enumerate(recipes, start=1):
    ...
    entries.append({
        'id': f'global_7_{index}', 'name': recipe['name_ar'],
        ...
        'tips': recipe.get('source_note') or recipe.get('source_url', 'وصفة تقليدية موثقة من مطبخ بلدها.'),
        'isIraqi': False, 'origin': 'general', 'image': image,
    })
content = "import type { Recipe } from './recipes';\n\nexport const GLOBAL_RECIPE_BATCH_SEVEN: Recipe[] = " + json.dumps(entries, ensure_ascii=False, indent=2) + ";\n"
(root / 'lib/data/global-recipe-batch-7.ts').write_text(content, encoding='utf-8')
print(len(entries))
