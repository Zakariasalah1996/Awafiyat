...
recipes = json.loads((root / 'data/recipe-batch-6.json').read_text(encoding='utf-8'))['recipes']
images = json.loads((root / 'data/recipe-batch-6-images.json').read_text(encoding='utf-8'))
...
for index, recipe in enumerate(recipes, start=1):
    ...
    entries.append({
        'id': f'global_6_{index}', 'name': recipe['name_ar'],
...
content = "import type { Recipe } from './recipes';\n\nexport const GLOBAL_RECIPE_BATCH_SIX: Recipe[] = " + json.dumps(entries, ensure_ascii=False, indent=2) + ";\n"
(root / 'lib/data/global-recipe-batch-6.ts').write_text(content, encoding='utf-8')
print(len(entries))
